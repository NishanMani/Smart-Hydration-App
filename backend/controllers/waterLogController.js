...
export const getHistoryInsights = async (req, res) => {
  try {
    ...
    const goalPerDay = req.user.dailyGoal > 0 ? req.user.dailyGoal : 2000;
    ...
    const totalLogs = await WaterLog.countDocuments({
      userId,
      date: { $gte: from, $lte: to },
    });

    const dailyTotals = await WaterLog.aggregate([
    ...
    const streakData = await WaterLog.aggregate([
    ...
    else if (streak >= 7) badge = "Consistency Star";
    else if (streak >= 3) badge = "Getting There";
